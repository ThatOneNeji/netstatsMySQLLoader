 ...
 * @property {array} skip List of service to not load
...
function initialiseApplication() {
    ...
    if (appConfig.skip) {
        appConfig.skip.forEach(element => {
            emptyArray.push(element.toLowerCase())
        });
    }
    appConfig.skip = emptyArray;
}
...
const pool = mysql.createPool({
    ...
    connectTimeout: appConfig.database.connectTimeout || 5
});
 ...
 *
 * @param {*} data String or object to write to disk
 * @param {object} payload Name of service that failed
 * @description This function writes the 'data' variable to a file
 */
function writeDataV2(data, payload, sql = '') {
    let filename = payload.rdate + '_' + payload.service + '.sql';
    let datastring;
    let filemessage;
    if (data.sql) {
        if (data.sql.constructor === String) {
...
function buildResultString(result) {
    let parsedresult = '';
    Object.keys(result).sort((a, b) => a.localeCompare(b)).forEach(function (key) {
 ...
 * @return {string} 
 */
function getRDate(line) {
    const regPattern = new RegExp(/\('(?<year>[\d]+)-(?<month>[\d]+)-(?<day>[\d]+)\s(?<hour>[\d]+)/gmi);
    const result = regPattern.exec(line);
    if (result) {
        return result.groups.year + result.groups.month + result.groups.day + result.groups.hour;
    }
...
function checkPayload(payload) {
    ...
    newPayload.service = payload.service.toLowerCase() || 'unknown';
    newPayload.load = !appConfig.skip.includes(newPayload.service);
    if (payload.sql) {
        newPayload.sql = payload.sql;
    }
    if (payload.content) {
        if (payload.content.sql) {
            newPayload.sql = payload.content.sql;
        }
        if (payload.content.name) {
            newPayload.sql = payload.content.name;
        }
    }

    newPayload.rdate = getRDate(newPayload.sql);
    if (!newPayload.rdate) {
        newPayload.rdate = moment().format('YYYYMMDDHH');
    }
    return newPayload;
}
...
function receiveHandler(queuemessage) {
    ...
    let payload = checkPayload(MySQLJSONPayload);
    if (payload.load) {
        return new Promise(function (resolve, reject) {
            ...
            pool.query(payload.sql, function (error, results, fields) {
                if (error) {
                    Logging.mysql.error('Service: "' + payload.service + '" results: ' + buildResultString(error));
                    writeDataV2(error, payload, payload.sql);
                    if (fields) {
                        Logging.mysql.debug(fields);
                    }
                    reject(error);
                } else {
                    parseSQLResults(payload.service, results);
                }
            });
        ...
        Logging.mysql.debug('Dataset \'' + payload.service + '\' set to not load');
        writeDataV2(payload, payload);
        return true;
    }
}
